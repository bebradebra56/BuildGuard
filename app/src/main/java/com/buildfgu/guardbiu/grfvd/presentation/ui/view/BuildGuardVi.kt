...
class BuildGuardVi(
    private val buildGuardContext: Context,
    private val buildGuardCallback: BuildGuardCallBack,
    private val buildGuardWindow: Window
) : WebView(buildGuardContext) {
    private var buildGuardFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildGuardSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildGuardFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(buildGuardContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildGuardContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(buildGuardContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    BuildGuardApplication.buildGuardInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(BuildGuardApplication.BUILD_GUARD_MAIN_TAG, "onPageFinished : ${BuildGuardApplication.buildGuardInputMode}")
                    buildGuardWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildGuardEnableStableInputScroll()
                } else {
                    BuildGuardApplication.buildGuardInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(BuildGuardApplication.BUILD_GUARD_MAIN_TAG, "onPageFinished : ${BuildGuardApplication.buildGuardInputMode}")
                    buildGuardWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildGuardFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                buildGuardHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun buildGuardFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildGuardEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun buildGuardHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = BuildGuardVi(buildGuardContext, buildGuardCallback, buildGuardWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildGuardCallback.buildGuardHandleCreateWebWindowRequest(windowWebView)
        }
    }
